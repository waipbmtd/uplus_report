...
(function ($) {
    ...
    function Listbox(domelement, options) {
        var settings = $.extend({
            ...
            'itemselect':  null,
            'itemunselect':  null,
            'doublecheck' : false,    //if true , second check means select
                                      // and review if it's not consecutive check
                                    // Only available when it is MutipleselectList
            'fndoublecheck': null   //A function only available when doublecheck is true
                                    // and it is MutipleSelectList
        }, options);

        this.preclick = null,     //previous click item value
        this.name   = null,        //which does this List belong to
    ...
    Listbox.prototype.getSelectValue = function () {
        return this._parent.val();
    };
    ...
    Listbox.prototype.newList = function (new_list,check_list) {
        var self = this;
        self._list.html('');
        self._parent.html('');
        $.each(new_list,function(index,value){
            var selected = true;
            if (check_list!=undefined && check_list instanceof Array && !$.CommonFn.itemInArray(value, check_list)){
                selected = false;
            }
            self.addItem($('<option>').attr({"value":value,"selected":selected})
                .text(value).appendTo(self._parent));
        })
    ...
    Listbox.prototype.addItem = function (parentItem) {
        var item = this.addItemHTML(parentItem);
        this.initClick(parentItem,item);
    };


    /**
     * Add item HTML to listbox
     * @param {Listbox}
     * @returns {*}
     */
    Listbox.prototype.addItemHTML = function (parentItem) {
        var self = this;
        var item = $('<div>')
     ...
     * @param item
     */
    Listbox.prototype.initClick = function (parentItem,item) {
        var self = this;
    ...
    SingleSelectListbox.prototype.onItemClick = function (item) {
        ...
        if (this._selected)
            ...
            if (this._settings.itemunselect)
                this._settings.itemunselect(this,item);
        this._selected = item.attr('selected', '');
        if (this._settings.itemselect)
            this._settings.itemselect(this,item);
    ...
    MultiSelectListbox.prototype.onItemClick = function (item) {
        ...
        var itemValue = parentItem.val();

        if (item.attr('selected') && this._settings.doublecheck){
            if (this.preclick != itemValue) {
                this._settings.fndoublecheck(this, item);
                this.preclick = itemValue;
                return
            }
        }
        ...
        if (item.attr('selected')) {
            ...
            var removeIndex = parentValue.indexOf(itemValue);
    ...
    $.fn.listbox = function (options) {
        if ($(this).attr('multiple'))
            var a = new MultiSelectListbox($(this), options);
            return a;
